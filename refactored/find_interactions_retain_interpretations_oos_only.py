...
def main(
    ...
    print(f"[INFO] Average Precision score for {input_data_type}, RETAIN interpretations OOS: "
          f"{average_precision_score(np.array(interaction_is_real), interaction_scores)}")
    disp = PrecisionRecallDisplay(precision=precision, recall=recall)
    disp.plot()
    plt.ylim(-0.05, 1.05)
    plt.title(f"PR curve for {input_data_type} OOS, RETAIN interpretations")
    plt.savefig(os.path.join(output_directory, f"{input_data_type}_retain_interpretations_oos_precision_recall.png"))
